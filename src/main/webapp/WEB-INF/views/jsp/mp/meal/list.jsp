...
<body>
	...
	<script type="text/javascript">
		...
		$("#nav h3").on('tap', function(e) {
			...
			var mealDate = this.id;
			
			/** 加载列表数据  **/
			$.ajax({
				url : $("base").attr("href") + "meal/listContent",
				type : "post",
				data : {
					mealDate : mealDate
				},
				...
				success : function(data) {
					...
					$("#"+ mealDate).addClass("on");
					$("#list").html(data);
					
					/** 绑定列表链接* */
					$("#list article").on('tap', function(e) {
						location.href = "meal/detail/" + this.id;
					});
				}
			});
			//location.href = "meal/list?mealDate=" + this.id;
		});
		...
		$('[data-toggle="datepicker"]').on('pick.datepicker', function(e) {
			var mealDate = e.date.format("yyyy-MM-dd");
			location.href = "meal/list?mealDate=" + mealDate;
			e.preventDefault();
		});
	</script>
</body>
